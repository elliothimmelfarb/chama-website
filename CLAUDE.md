...
2. **Never connect this repo (or the brain) to Vercel's git integration.** Deploys are always the manual CLI run below. Committing never deploys anything by itself. If this repo ever gets a remote, that decision is Elliot's; a public GitHub remote is compatible with the split, git integration with the host is not.
