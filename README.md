...
| `assets/figures.css`, `assets/figures.js` | The section figures' shared motion system: the draw, fade, pop, ring, comet-head, ghost, hug, orbit, glow and flare classes, and the observer that arms the page and starts each figure the first time it is seen (a tap replays it). Shared by `index.html` (six figures) and `privacy.html` (one: where your words go). Without JavaScript or under reduced motion every figure is simply complete. |
