...
| File | What it is |
|---|---|
| `index.html` | The public home page. Since 2026-09-21 (the owners' copy review with Annie, see `docs/reports/2026-09-21-homepage-owners-review.html`) it opens on "Software shaped for your business" with a hero figure that tells the fit story, states the offer as "We build and support. You own." in three parts, and explains the process (four questions, a first version, the subscription carries on). The register is plain whole sentences; the retired lines ("Your process, your words, your rules", "Your business, your rules", "whenever you ask", the export line, the olden-days framing) must not come back. Its second panel is the flame room: the intelligent flame, the site's live chat agent, embedded full-bleed directly under the hero, opening with the question "What do you wish your software could do?" and taking a name and one contact way when the visitor is ready. Embedded, the room hides its session readout and tune button (the page is for the visitor's business, and the flame's tricks stay an easter egg the agent still performs when asked). No name appears on the page; the people are on `/about`. No external fonts, images, scripts, analytics, or trackers. |
| `coaching.html`, `about.html` | The two sub pages at `/coaching` (coaching one to one and team training; moved off the home page on 2026-09-03 as `/training`, renamed and rewritten coaching-first on 2026-09-24, and `/training` now redirects to it permanently in `vercel.json`) and `/about` (the founder and About us sections). Same visual language, indexable, in the sitemap. Each ends in a dark band linking to `/agent`. The Members pill (the way into the Hearth) lives only on `/coaching`, beside the coaching it belongs to. |
| `assets/site.css` | The public pages' shared stylesheet, extracted from `index.html` on 2026-09-03 so the three pages share one design. `privacy.html` and `404.html` keep their own inline styles, with the footer rules copied in. Every public page (`/`, `/coaching`, `/about`, `/privacy`, and the 404) ends in the same footer since 2026-09-14: company line, the company LinkedIn, and the site map `Software · Training and coaching · About · Privacy` with the current page marked `aria-current`. It is company-only on purpose: Elliot's GitHub lives in the founder pill on `/about`, and the Members pill stays on `/training`. Change the footer in all five files together; there is no include step. |
...
| `assets/agent.js`, `assets/agent.css` | The agent app itself, extracted so both pages share it: `ChamaAgent.mount(root, { mode: "page" | "embed" })`. Canvas particle flame, chat panel, HUD, tune panel. Vanilla, no libraries. Embed mode is container-scoped (container queries, root-relative geometry, pointer events bound to the root), pauses its animation loop off-screen, and on pointer devices holds the transcript closed as a scroller until the room is seated (its bottom edge on the bottom of the viewport, reached by routing the wheel by hand). On a touch device the room is never a nested scroller: in the page it is a static panel, and engaging it (focusing the composer, or tapping a conversation under way) pins it to the visual viewport as a sheet (`kb-pin`, `kb-open`, `html.room-pinned`) for the whole conversation, so the transcript is the only scroller and a soft keyboard cannot walk the layout off the top; Back to page takes the sheet down and restores the scroll position (since 2026-09-14, replacing the scroll snap seat and hand-routed chaining). The opening carries only the question and the composer; there are no starter chips. The tune is never persisted: every load starts from defaults. |
| `assets/figures.css`, `assets/figures.js` | The section figures' shared motion system: the draw, fade, pop, ring, comet-head, ghost, hug, orbit, glow and flare classes, and the observer that arms the page and starts each figure the first time it is seen (a tap replays it). Shared by `index.html` (two figures), `coaching.html` (four: the session spiral in the hero, the learning curve, arrive and leave, and the model releases), and `privacy.html` (one: where your words go). Without JavaScript or under reduced motion every figure is simply complete. Below 520px the wide figures re-stack (each column is a group; CSS transforms in the phone block set them one under another, the box takes its height from `--stack-h`, gated on `aspect-ratio` support) and the sub pages' charts raise their type, with two-line phone captions (`.ph`) beside the one-line desktop ones (`.dt`). |
| `assets/view.js` | The Words / Pictures switch (since 2026-09-22, [issue #59](https://github.com/elliothimmelfarb/chama-website/issues/59), report `docs/reports/2026-09-22-pictures-view.html`). The public pages have two views of the same copy, toggled from the masthead the way light and dark are. Words is the page as written. Pictures ("big pictures, few words", after a line of Thariq's from the Claude Code team) keeps the headlines, the figures, the four questions, and the links, hides the explanatory paragraphs, and sets each section as a poster: the kicker in ember, the headline at poster size, the figure at page width. The view is `html[data-view="pictures"]`, set before first paint by an inline script in each page's head from localStorage (`chama-view`) or from `?view=pictures` / `?view=words` in the address, which also sets the memory; `view.js` only wires the `role="switch"` button, runs the flip as a view transition (the masthead, every headline and every figure carry an inline `view-transition-name`, unique per page), and asks the visible figures to redraw (`chama:view` event, handled in `figures.js`). The rules are the block at the end of `site.css`. Everything Pictures shows is already on the Words page: one figure (the offer in three shapes) and two lines exist only for that view (`.pictures-only`) and quote the copy they stand in for. Privacy has no switch: legal text is not a place to drop words. The choice never leaves the device. |
...
| `brand/og/` | Source of the link preview card (`og-card.html`, plain HTML on the site's own tokens and font stack) and `render.sh`, which screenshots it with headless Chrome at 1200x630. The output is a dated root file (`og-2026-09-22.png`) referenced by every page's `og:image` and `twitter:image`. Messengers cache a preview by URL for a long time, so a changed card is shipped as a new filename, never over the old one; the old `og.png` stays for links already sent. Copy on the card is canonical in the brain's website page. |
