...
export const MIGRATIONS = [
  ...
  },
  {
    ...
    statements: [
      ...
      `alter table bookings add column if not exists meeting_code text`,
      `alter table bookings add column if not exists meet_attempts int not null default 0`,
      `alter table bookings add column if not exists meet_checked_at timestamptz`
    ]
  },
  {
    id: "004-one-transcript-per-booking",
    statements: [
      // A session has one record. Two cold instances, or the cron and the
      // owner at once, used to be able to write a second one. If a duplicate
      // already exists the index cannot be built, and a migration that fails
      // on every cold start would take the whole room down; so the index is
      // skipped, the writers keep their check-then-insert as the first line,
      // and the README says how to resolve the duplicates and add the index
      // by hand.
      `do $$
      begin
        if exists (
          select 1 from transcripts where booking_id is not null
          group by booking_id having count(*) > 1
        ) then
          raise warning 'transcripts_booking_idx skipped: a booking has more than one transcript';
        else
          create unique index if not exists transcripts_booking_idx
