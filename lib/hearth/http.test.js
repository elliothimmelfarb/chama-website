...
// Nothing a visitor typed carries a control character onward: a NUL ends a
// Postgres string, a newline forges a header in an email subject.
test("clampText drops control characters and truncates", () => {
  assert.equal(clampText("a\u0000b\u0000"), "ab");
  assert.equal(clampText("abcdef", 3), "abc");
  assert.equal(clampText(42), "");
  // A newline in a name would be a second header in an email subject.
  assert.equal(clampText("Elliot\r\nBcc: someone@example.com"), "Elliot Bcc: someone@example.com");
  assert.equal(clampText("a\u0007b\u007fc"), "abc");
  // Prose keeps its line breaks and loses everything else.
  assert.equal(clampText("one\r\ntwo\u0000\u0007", 100, { multiline: true }), "one\ntwo");
});
