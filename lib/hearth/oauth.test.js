...
test("denying sends the person back with access_denied and the state", async () => {
  useClient(
    fakeDb([
      [/select meta from oauth_states/, [{ meta: pendingMeta }]],
      [/delete from oauth_states/, [{ meta: pendingMeta }]]
    ])
  );
  const result = await denyAuthorization("req-1", "session-1");
  const url = new URL(result.redirect);
  assert.equal(url.searchParams.get("error"), "access_denied");
  assert.equal(url.searchParams.get("state"), "xyz");
});

// A request belongs to the browser that opened it. Another member holding
// the id cannot answer for it, in either direction.
test("a deny from a different browser session is refused and leaves the request standing", async () => {
  const db = fakeDb([
    [/select meta from oauth_states/, [{ meta: { ...pendingMeta, sessionId: "session-1" } }]],
    [/delete from oauth_states/, [{ meta: pendingMeta }]]
  ]);
  useClient(db);
  assert.equal(await denyAuthorization("req-1", "session-2"), null);
  assert.equal(db.count(/delete from oauth_states/), 0);
});
