...
export function registerAgentRoutes({ route, needs }) {
  ...
  route("POST", "/agents/keys", async (context) => {
    ...
    if (!scopes.length) throw new HttpError(400, "Select at least one permission for the key.");
    if (scopes.includes("business") && !context.actor.permissions.has("members.read")) throw new HttpError(403, "Your role does not include business access.");
    const count = await sql()`select count(*)::int as n from api_keys where user_id = ${context.actor.user.id} and revoked_at is null`;
    if ((count[0]?.n || 0) >= 20) throw new HttpError(400, "You can have at most 20 active keys. Revoke one first.");
    let expiresAt = null;
  ...
  route("GET", "/agents/consent", async (context) => {
    ...
    if (!context.session) throw new HttpError(403, "Open this page in the browser that started the connection.");
    const pending = await readAuthorization(url.searchParams.get("req"), context.session.id);
  ...
  route("POST", "/agents/consent", async (context) => {
    ...
    if (body.decision !== "approve") {
      const denied = await denyAuthorization(req, context.session.id);
    ...
    const approved = await approveAuthorization(req, context.actor.user.id, scopes, context.session.id);
