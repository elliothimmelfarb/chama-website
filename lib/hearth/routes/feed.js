  ...
  route("GET", "/feed", async (context) => {
    ...
    const rows = await sql()`
      ...
      where (${canWrite} or (p.published_at is not null and p.visibility = any(${levels})))
        and (${before}::timestamptz is null or p.published_at < ${before}::timestamptz or (p.published_at is null and ${canWrite}))
...
function validPost(body) {
  ...
  return {
    title,
    body: clampText(body.body, 20000, { multiline: true }),
    url,
