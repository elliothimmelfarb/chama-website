...
test("saying no sends the app away with access_denied", async () => {
  useClient(signedIn({ extra: [[/select meta from oauth_states/, [{ meta: pendingMeta }]], [/delete from oauth_states/, [{ meta: pendingMeta }]]] }));
...
test("saying yes hands the app a code, and a client cannot grant the business", async () => {
  const db = signedIn({ extra: [[/select meta from oauth_states/, [{ meta: pendingMeta }]], [/delete from oauth_states/, [{ meta: pendingMeta }]]] });
  useClient(db);
