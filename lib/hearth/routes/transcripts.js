...
import { isUniqueViolation, sql } from "../db.js";
...
export function registerTranscriptRoutes({ route, needs }) {
  ...
  route("POST", "/notes", async (context) => {
    ...
    const body = await readJson(context.request);
    const text = clampText(body.text, 4000, { multiline: true }).trim();
  ...
  route("POST", "/admin/transcripts", async (context) => {
    ...
    // The check above is a courtesy; the unique index is what decides.
    let rows;
    try {
      rows = await sql()`
      ...
      if (isUniqueViolation(error)) throw new HttpError(409, "That session already has a transcript.");
      throw error;
    }
    const t = rows[0];
