...
export function registerTranscriptRoutes({ route, needs }) {
  ...
  route("POST", "/follow-ups", async (context) => {
    ...
    if (!text) throw new HttpError(400, "Write the follow-up first.");
  ...
  route("POST", "/admin/transcripts", async (context) => {
    needs(context, "transcripts.manage");
    if (!(await allow(`derive:${context.actor.user.id}`, 30, 3600))) throw new HttpError(429, "That is a lot of transcripts for one hour. Try again a little later.");
    ...
    if (raw.length < 40) throw new HttpError(400, "That transcript is too short to be one.");
  ...
  route("POST", "/admin/transcripts/:id/derive", async (context, params) => {
    needs(context, "transcripts.manage");
    if (!(await allow(`derive:${context.actor.user.id}`, 30, 3600))) throw new HttpError(429, "That is a lot of records for one hour. Try again a little later.");
