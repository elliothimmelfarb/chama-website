...
export function registerSessionRoutes({ route, needs, readSettings }) {
  ...
  route("POST", "/admin/credits", async (context) => {
    ...
    if (!Number.isInteger(delta) || delta === 0 || Math.abs(delta) > 100) throw new HttpError(400, "A whole number of credits, please.");
  ...
  route("GET", "/admin/bookings", async (context) => {
    ...
    `;
    return json({ bookings: rows.map((b) => bookingView(b, true)) });
  });
  ...
  route("POST", "/admin/bookings", async (context) => {
    ...
    if (!body.free) {
      const spent = await sql()`
        ...
        returning id
      `;
      if (!spent[0]) throw new HttpError(402, "They have no credits. Tick 'free' to book without one.");
      creditId = spent[0].id;
    }
    let booking;
    try {
