...
export function registerSessionRoutes({ route, needs, readSettings }) {
  ...
  route("POST", "/purchases", async (context) => {
    ...
    const rows = await sql()`
      ...
      values (${context.actor.user.id}, ${pack.id}, ${pack.name}, ${pack.sessions}, 'requested', ${amount}, ${discountCents}, ${pack.currency}, ${clampText(body.note, 1000, { multiline: true })})
      returning *
    `;
  ...
  route("POST", "/bookings", async (context) => {
    ...
    beyondHorizon(settings, startsAt);
    ...
    try {
      const rows = await sql()`
        ...
        values (${user.id}, ${startsAt.toISOString()}, ${endsAt.toISOString()}, 'scheduled', ${spent[0].id}, ${clampText(body.note, 2000, { multiline: true })}, ${settings.meeting_url || null})
        returning *
      `;
      booking = rows[0];
  ...
  route("POST", "/bookings/:id/cancel", async (context, params) => {
    ...
    // The cancel is the claim: only the caller whose update finds the booking
    // still scheduled refunds the credit, so two cancels at once mint one.
    const cancelled = await sql()`
      update bookings set status = 'cancelled', cancelled_at = now(), cancel_reason = ${clampText(body.reason, 500)}
      where id = ${booking.id} and status = 'scheduled' returning id
    `;
    if (!cancelled[0]) throw new HttpError(400, "That session is not scheduled.");
  ...
  route("PATCH", "/admin/purchases/:id", async (context, params) => {
    ...
    if (typeof body.status === "string" && body.status !== purchase.status) {
      ...
      if (body.status === "paid") {
        ...
        await log(body.status === "invoiced" ? SESSION_EVENTS.packInvoiced : SESSION_EVENTS.packRequested, purchase.id, { status: body.status });
      }
    }
    return json({ ok: true });
  });
  ...
  route("POST", "/admin/bookings", async (context) => {
    ...
    try {
      const rows = await sql()`
        ...
        values (${member.id}, ${startsAt.toISOString()}, ${endsAt.toISOString()}, 'scheduled', ${creditId}, ${clampText(body.note, 2000, { multiline: true })}, ${clampText(body.title, 200)}, ${settings.meeting_url || null})
        returning *
      `;
      booking = rows[0];
  ...
  route("PATCH", "/admin/bookings/:id", async (context, params) => {
    ...
    if (typeof body.status === "string" && body.status !== booking.status) {
      ...
      // Compare and set on the status that was read, so two owners changing
      // one booking at once cannot both refund it.
      const changed = await sql()`
        update bookings set status = ${body.status}, cancelled_at = ${body.status === "cancelled" ? new Date().toISOString() : null}
        where id = ${booking.id} and status = ${booking.status} returning id
      `;
      if (!changed[0]) throw new HttpError(409, "That session changed while you were looking at it. Reload and try again.");
    ...
    if (typeof body.ownerNote === "string") await sql()`update bookings set owner_note = ${clampText(body.ownerNote, 4000, { multiline: true })} where id = ${booking.id}`;
...
function validPack(body) {
  ...
  return {
    name, description: clampText(body.description, 600, { multiline: true }), sessions, minutes, priceCents, currency,
    active: body.active !== false, position: Number.isInteger(body.position) ? body.position : 0
  };
}

// The horizon the slot list already respects, applied to a booking too, so a
// request that skips the list cannot reach past it.
function beyondHorizon(settings, startsAt) {
  const days = settings.booking_horizon_days;
  if (startsAt.getTime() <= Date.now() + days * 86400000) return;
  throw new HttpError(400, `Sessions can be booked up to ${days} day${days === 1 ? "" : "s"} ahead.`);
}
...
async function rewardReferrer(purchase, context) {
  ...
  // Claim the event before rewarding it: whoever's update finds it still
  // signed up is the one caller that pays out.
  const claimed = await sql()`
    update referral_events set status = 'rewarded', rewarded_at = now()
    where id = ${event.id} and status = 'signed_up' returning id
  `;
  if (!claimed[0]) return;
  let reward;
  try {
    ...
    reward = { type: settings.type, amount: Number(settings.amount) || 0, purchase: purchase.id };
    ...
    await sql()`update referral_events set reward = ${JSON.stringify(reward)}::jsonb where id = ${event.id}`;
  } catch (error) {
    // The claim is only worth keeping if the payout followed it. Put the event
    // back the way it was so a later purchase can still reward it, then let the
    // failure travel. Claiming again is the same compare-and-set, so a
    // concurrent caller still cannot pay twice.
    await sql()`
      update referral_events set status = 'signed_up', rewarded_at = null
      where id = ${event.id} and status = 'rewarded'
    `;
    throw error;
  }
