...
export function registerAskRoutes({ route, needs }) {
  route("POST", "/ask", async (context) => {
    ...
    const question = clampText(body.question, 4000, { multiline: true }).trim();
    if (!question) throw new HttpError(400, "Enter a question.");
    const history = Array.isArray(body.history)
      ? body.history.slice(-8).filter((m) => m && (m.role === "user" || m.role === "assistant") && typeof m.content === "string").map((m) => ({ role: m.role, content: clampText(m.content, 6000, { multiline: true }) }))
      : [];
