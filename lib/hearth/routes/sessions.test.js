...
test("cancelling in good time gives the credit back", async () => {
  const db = signedIn({
    extra: [
      ...
      [/update bookings set status = 'cancelled'/, [{ id: "booking-1" }]],
...
test("two cancels of one session refund one credit", async () => {
  // The guarded update finds the booking scheduled once; the second caller
  // gets no row back, so no second refund is written.
  let claims = 0;
  const db = signedIn({
    extra: [
      [/select \* from bookings where id/, [bookingRow(SLOT_START)]],
      [/update bookings set status = 'cancelled'/, () => (++claims === 1 ? [{ id: "booking-1" }] : [])],
      [/as balance from credit_ledger/, [{ balance: 1 }]],
      [/select email, name from users where role = 'owner'/, []]
    ]
  });
  ...
  const cancel = () =>
    handleHearth(hearth("/bookings/booking-1/cancel", { method: "POST", cookie: "t", body: JSON.stringify({}) }));
  const [first, second] = await quiet(() => Promise.all([cancel(), cancel()]));
  const statuses = [first.status, second.status].sort();
  assert.deepEqual(statuses, [200, 400]);
  assert.equal(db.count(/insert into credit_ledger/), 1);
});

test("a session past the booking horizon is refused before the slots are read", async () => {
  const db = bookingDb({ settings: [{ key: "booking_horizon_days", value: 1 }] });
  useClient(db);
  const response = await handleHearth(
    hearth("/bookings", { method: "POST", cookie: "t", body: JSON.stringify({ startsAt: SLOT_START.toISOString() }) })
  );
  assert.equal(response.status, 400);
  assert.equal((await body(response)).error, "Sessions can be booked up to 1 day ahead.");
  assert.equal(db.count(/insert into bookings/), 0);
  assert.equal(db.count(/insert into credit_ledger/), 0);
});
...
test("a referral reward that cannot be paid does not stay claimed", async () => {
  const db = signedIn({
    role: "owner",
    users: { "member-1": MEMBER },
    extra: [
      [/select \* from purchases where id/, [PURCHASE]],
      [/from referral_events where referred_id/, [{ id: "referral-1", referrer_id: "referrer-1", status: "signed_up" }]],
      [/update referral_events set status = 'rewarded'/, [{ id: "referral-1" }]],
      [
        /insert into credit_ledger/,
        (call) => {
          if (/'referral'/.test(call.text)) throw new Error("LedgerWriteFailed");
          return [];
        }
      ]
    ]
  ...
  const reset = db.matching(/update referral_events set status = 'signed_up'/);
  assert.equal(reset.length, 1, "the claim is put back so a later purchase can still reward it");
  assert.deepEqual(reset[0].values, ["referral-1"]);
  assert.equal(db.count(/update referral_events set reward/), 0);
});
