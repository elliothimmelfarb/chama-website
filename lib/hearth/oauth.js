...
// The first browser session to open a pending request owns it: nobody else
// can render or approve it afterwards, so a request minted by an attacker
// cannot be handed to a victim.
export async function readAuthorization(requestId, sessionId = null) {
  ...
  if (sessionId) {
    if (meta.sessionId && meta.sessionId !== sessionId) return null;
    if (!meta.sessionId) {
      await sql()`update oauth_states set meta = meta || ${JSON.stringify({ sessionId })}::jsonb where state = ${requestId} and provider = 'oauth'`;
...
export async function approveAuthorization(requestId, userId, grantedScopes, sessionId = null) {
  const rows = await sql()`delete from oauth_states where state = ${requestId} and provider = 'oauth' and expires_at > now() returning meta`;
  if (!rows[0]) return null;
  const meta = rows[0].meta;
  if (meta.sessionId && sessionId && meta.sessionId !== sessionId) return null;
...
// Saying no is the same ownership question as saying yes: only the browser
// session the request belongs to may answer it, and only that browser gets
// the redirect back, state and all.
export async function denyAuthorization(requestId, sessionId = null) {
  const found = await sql()`select meta from oauth_states where state = ${requestId} and provider = 'oauth'`;
  if (!found[0]) return null;
  if (found[0].meta.sessionId && sessionId && found[0].meta.sessionId !== sessionId) return null;
