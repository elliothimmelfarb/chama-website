...
// The limit is octets of UTF-8, and a fold never lands inside a character.
test("a line of non-ASCII text folds by weight, not by character count", () => {
  const summary = "Sess\u00e3o com Elliot sobre rea\u00e7\u00e3o dire\u00e7\u00e3o e \u00e9nfase \u{1f525} ".repeat(4).trim();
  const ics = icsEvent({ ...BASE, summary });
  for (const line of ics.split("\r\n")) {
    assert.ok(new TextEncoder().encode(line).length <= 75, `line of ${new TextEncoder().encode(line).length} octets: ${line}`);
  }
  assert.ok(lines(ics).includes(`SUMMARY:${summary}`), "unfolding gives back the original line");
  assert.ok(!/\ufffd/.test(ics), "no character was cut in half");
});
