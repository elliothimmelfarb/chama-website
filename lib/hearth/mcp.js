...
// The same, for a field the room shows as prose: line breaks survive.
function prose(v, max = 2000) { return clampText(typeof v === "string" ? v : "", max, { multiline: true }).trim(); }
...
tool({
  name: "whoami",
  scope: "profile",
  description: "Who this token belongs to: name, email, role, timezone, and what the token can do (its permissions and granted scopes). Call it first to learn the member's timezone before quoting any session time. Takes no arguments.",
  inputSchema: { type: "object", properties: {}, additionalProperties: false },
...
tool({
  ...
  scope: "sessions",
  perm: "sessions.own",
  description: "The member's session credit balance and the last twenty ledger entries (delta, reason, note, date). A booking spends one credit; a cancellation inside the notice window returns it. Takes no arguments.",
  inputSchema: { type: "object", properties: {}, additionalProperties: false },
  run: async (args, caller) => {
...
tool({
  ...
  inputSchema: { type: "object", properties: { which: { type: "string", enum: ["upcoming", "past", "all"], default: "all", description: "upcoming: scheduled sessions that have not ended; past: everything else; all: both. Newest first, at most 100." } }, additionalProperties: false },
  run: async (args, caller) => {
...
tool({
  ...
  inputSchema: { type: "object", properties: { id: { type: "string", description: "A record id from list_records or list_sessions (recordId)." }, includeTranscript: { type: "boolean", default: false, description: "Also return the raw transcript. It can be long; leave false unless the transcript itself is needed." } }, required: ["id"], additionalProperties: false },
  run: async (args, caller) => {
...
tool({
  ...
  inputSchema: { type: "object", properties: { query: { type: "string", description: "Plain words, at least two characters; matched against transcripts and records with English full-text search. Returns at most 20 sessions, newest first." } }, required: ["query"], additionalProperties: false },
  run: async (args, caller) => {
...
tool({
  ...
  inputSchema: { type: "object", properties: { openOnly: { type: "boolean", default: true, description: "True returns only follow-ups not yet done; false includes done ones." } }, additionalProperties: false },
  run: async (args, caller) => {
...
tool({
  ...
  inputSchema: { type: "object", properties: { id: { type: "string", description: "A follow-up id from list_follow_ups." }, done: { type: "boolean", default: true, description: "True marks it done now; false reopens it." } }, required: ["id"], additionalProperties: false },
  run: async (args, caller) => {
...
tool({
  ...
  run: async (args, caller, request) => {
    const response = await callHearth(request, caller, "POST", "/bookings", { startsAt: args.startsAt, note: prose(args.note, 2000) });
...
tool({
  ...
  inputSchema: { type: "object", properties: { id: { type: "string", description: "A session id from list_sessions." }, reason: { type: "string", description: "Optional, up to 500 characters; stored as the member's words for Elliot to read." } }, required: ["id"], additionalProperties: false },
...
tool({
  ...
  inputSchema: { type: "object", properties: { title: { type: "string", description: "Up to 200 characters; also becomes the post's slug." }, body: { type: "string", description: "Plain text, up to 20000 characters. Not markdown." }, url: { type: "string", description: "Optional http(s) link the post points at." }, visibility: { type: "string", enum: ["public", "members", "clients"], default: "members", description: "Who can read it: public (anyone, if the public feed is on), members (signed-in members), clients (clients only)." }, kind: { type: "string", enum: ["note", "link", "brief"], default: "note", description: "note: a thought; link: the url is the point; brief: a longer write-up." } }, required: ["title"], additionalProperties: false },
  run: async (args, caller) => {
    ...
    const rows = await sql()`
      insert into feed_posts (slug, author_id, title, body, url, kind, visibility, published_at) values (${slug}, ${caller.user.id}, ${title}, ${prose(args.body, 20000)}, ${url}, ${kind}, ${visibility}, now()) returning id, slug
    `;
...
tool({
  ...
  inputSchema: { type: "object", properties: { memberEmail: { type: "string", description: "The member's sign-in email; the transcript attaches to that member." }, text: { type: "string", description: "The transcript as plain text, WebVTT, or SRT; Speaker: line shapes are recognised." }, heldAt: { type: "string", description: "ISO 8601 date-time the session was held; defaults to now." }, title: { type: "string", description: "Optional title, up to 200 characters; otherwise the record's own title is used." }, bookingId: { type: "string", description: "Optional booking id to attach to; attaching marks that booking completed." } }, required: ["memberEmail", "text"], additionalProperties: false },
  run: async (args, caller, request) => {
